...
cd "$directory" || { echo "Cannot cd into $directory"; exit 1; }
...
###############################################################################
# In-place edits of info.cfg (works on GNU and BSD sed)
###############################################################################
...
sed "${INPLACE[@]}" \
    "s|\"version\": *\"$current\"|\"version\": \"$version\"|g" info.cfg

sed "${INPLACE[@]}" \
    "s|\"date\": *\"[0-9]\{4\}-[0-9]\{2\}-[0-9]\{2\}\"|\"date\": \"$today\"|" info.cfg

###############################################################################
# Commit & push
###############################################################################
if $run; then
  git add VERSION info.cfg
